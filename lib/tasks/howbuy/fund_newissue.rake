namespace :howbuy do
  # 正在发行。和即将发行的不同之处是：数据源和发行状态不同
  desc "Task description"
  task :fetch_howbuy_fund_newissue_zzfx => [:environment] do
    # 数据源不同
    url = "http://www.howbuy.com/fund/fundnewissue.htm?pageType=zzfx"
  ...
  # 即将发行。和正在发行的不同之处是：数据源和发行状态不同
  task :fetch_howbuy_fund_newissue_jjfx => [:environment] do
    # 数据源不同
    url = "http://www.howbuy.com/fund/fundnewissue.htm?pageType=jjfx"
  ...
  task :save_newissue_to_dir => [:environment] do
    url_hash = {zzfx: "http://www.howbuy.com/fund/fundnewissue.htm?pageType=zzfx",
                jjfx: "http://www.howbuy.com/fund/fundnewissue.htm?pageType=jjfx"}

    url_hash.each_pair do |file_name, url|
      sb ||= SpiderBase.new
      fetch_content = sb.page_for_url(url);
      ...
      file_name_with_path = newissue_dir.join("#{file_name}.html")
